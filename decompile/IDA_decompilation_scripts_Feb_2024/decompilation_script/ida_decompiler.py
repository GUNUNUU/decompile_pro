...
def main():
	...
	if init_hexrays():
		...
		print("len(process_order):",len(process_order))
		#get_func_1_ea()
		...
		for funcea in Functions():
			...
			if index != -1:
				...
				# print("process_func_ea_inOrder["+str(index)+"]:",process_func_ea_inOrder[index])

		#with open(cpath, "w") as outfile:
			#print("writing results to '%s'..." % cpath)
		decompiled_result = os.path.join(decompiled, os.path.basename(idbpath[:-4]))
		os.system("mkdir -p " + decompiled_result)
		# print("len(process_func_ea_inOrder):",len(process_func_ea_inOrder))
		# try:
		for i in range(len(process_func_ea_inOrder)):
			# print(i)
			if process_func_ea_inOrder[i]:	# maliha added condition
